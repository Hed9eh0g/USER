...
public class AlgoUSER {
                ...
                for (int j = 0; j < itemset.size(); j++) {
                    ...
                    for (int k = i + 1; k < sequence.getItemsets().size(); k++) {
                        ...
                        for (ItemWithUtilityPosition itemY : itemsetK) {
                            ...
                            RuleInformation.utility = itemX.utility() + itemY.utility();

                            RuleInformation.AlphaPosition = itemX.position();
                            RuleInformation.BetaPosition = itemY.position();
                ...
                for (Entry<Integer, List<UtilityAndPostionAndSequenceId>> entryYInformation : entreeX.getValue().entrySet()) {
                    ...
                    for (UtilityAndPostionAndSequenceId StructureXY : ListStructureXY) {
                        if (SameSequenceId.get(StructureXY.SequenceId) == null) {
                            ...
                            System.out.print("\n    sequence id: " + StructureXY.SequenceId + "  (alpha: " + StructureXY.AlphaPosition + ", beta: " + StructureXY.BetaPosition + ")");
                        } else {
                            System.out.print("  (alpha: " + StructureXY.AlphaPosition + ", beta: " + StructureXY.BetaPosition + ")");
                        }
                    }
                    System.out.print("\n");
                }
        ...
        for (Entry<Integer, Map<Integer, List<UtilityAndPostionAndSequenceId>>> entryX : mapAlphaBeta.entrySet()) {
            ...
            for (Entry<Integer, List<UtilityAndPostionAndSequenceId>> entryYUtility : entryX.getValue().entrySet()) {
                ...
                for (UtilityAndPostionAndSequenceId StructureXY : ListStructureXY) {
                    ...
                    int ItemYPosition = StructureXY.BetaPosition;
                    ...
                    for (int i = ItemXPosition + 1; i < ItemYPosition; i++) {
                        ...
                        for (ItemWithUtilityPosition Item : Itemset) {
                            if (Item.getItemName() > ItemX && Item.getItemName() > ItemY) {
                                ...
                                // item.PrePosition < ItemXPosition && Item.NextPosition = -1
                                // item.PrePosition < ItemXPosition && Item.NextPosition >= ItemYPosition
                                ...
                                // After the previous screening step, the remaining cases include：
                                // item > itemX && item <= itemY, If we don't focus on the equal sign, it will represent the LeftExpandable
                                // item <= itemX && item > itemY, If we don't focus on the equal sign, it will represent the RightExpandable
                                // item <= itemX && item <= itemY, unExpandable
                    ...
                    table.addElement(element, SequenceId);
                }
                        ...
                        for (int w = itemsetZ.size() - 1; w >= 0; w--) {
                            ...
                            if (itemW.getItemName() < itemJ.getItemName()) {
                                ...
                                if (itemW.getItemName() < largestItemInAntecedent) {
                                    // This means that even if it's in area II, it's still RightExpandable
                                    ...
                                    // This means itemW becomes LeftRightExpandable in area II
                                    double MaxProfitItemW1 = test.MaxUtilityItemBetweenAlphaAndBeta(itemW, itemW.position(), sequence.size() - 1, sequence, 1).utility();
                        ...
                        for (int w = itemsetZ.size() - 1; w >= 0; w--) {
                            ...
                            if (wIsLeftRight && itemW.getItemName() <= itemJ.getItemName()) {
                                ...
                                if (!itemW.getItemName().equals(itemJ.getItemName())) {
                                    // If it is equal to itemJ, then it cannot be LeftExpandable subsequently
                                ...
                                if (itemW.NextPosition == -1) {
                                    // Traverse and find the last duplicate item
                                    // If occurrence exists in area I
                                    double MaxProfitItemW1 = test.MaxUtilityItemBetweenAlphaAndBeta(itemW, element.positionAlphaItemset + 1, itemW.position(), sequence, 0).utility();
                                    newElement.utilityRight -= Math.max(itemW.utility(), MaxProfitItemW1);
                                }
                            }
                ...
                for (int j = 0; j < itemsetI.size(); j++) {
                    ...
                    for (int z = element.positionBetaItemset; z < sequence.size(); z++) {
                        ...
                        for (int w = itemsetZ.size() - 1; w >= 0; w--) {
                            ...
                            if (itemW.getItemName() <= itemJ.getItemName()) {
                                ...
                                if (itemW.getItemName() < largestItemInAntecedent) {
                                    // possible cases：
                                    // （1）in area I + area II + area III
                                    // （2）in area I + area II
                                    // （3）in area I + area III
                                    // （4）only in area I
                                    if (itemW.getItemName().equals(itemJ.getItemName())) {
                                        continue;
                                    }
                    ...
                    for (int z = element.positionBetaItemset - 1; z >= i; z--) {
                        ...
                        for (int w = itemsetZ.size() - 1; w >= 0; w--) {
                            ...
                            // Determine whether there is a duplicate with the item in Alpha
                            Duplicate = false;
                            for (int value : antecedent) {
                                if (itemW.getItemName().equals(value)) {
                                    Duplicate = true;
                            ...
                            if (wIsRight && itemW.getItemName() < itemJ.getItemName()) {
                                ...
                                if (itemW.NextPosition == -1) {
                                    // Only exist in II, or II and III
                                ...
                                // LeftRightExpandable becomes RightExpandable
                                ItemWithUtilityPosition itemZ = itemW;
                                boolean flag = false;
                                while (itemZ.PrePosition != -1) {
                                    int PrePosition = itemZ.PrePosition;
                                ...
                                // Left and right expansion becomes non-expandable
                                // Regardless of whether only in II area, or II and III, it is removed
                                // If it still exists in III area (correspondingly, the left and right expansion becomes the left expansion), then it will be added in area III later
                                double MaxProfitItemW1 = test.MaxUtilityItemBetweenAlphaAndBeta(itemW, element.positionAlphaItemset + 1, itemW.position(), sequence, 0).utility();
                                newElement.utilityLeftRight -= Math.max(itemW.utility(), MaxProfitItemW1);
                            } else if (wIsLeft) {
                                // Left expandable becomes non-expandable (due to position)
                    ...
                    // Scan between alpha and betaprime (area II')
                    ItemHasBeenSeem = new HashMap<>();
    ...
    public void printStats(String input) {
        System.out.println("=============== USER algorithm ===================");
